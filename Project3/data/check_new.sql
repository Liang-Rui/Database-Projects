...
-- Q7
create or replace function
	proj1_check_q7() returns text
as $$
declare
	dis text;
	nondis text;
begin
	dis := proj1_check('view', 'q7', 'q7_expected_distinct', 'select * from q7');
	nondis := proj1_check('view', 'q7', 'q7_expected', 'select * from q7');
	if (dis = 'correct' or nondis = 'correct') then
		return 'correct';
	elsif (dis = nondis) then
		return dis;
	else
		return dis||' or '||nondis;
	end if;
	return '???';
end;
$$ language plpgsql;
...
drop table if exists q7_expected_distinct;
create table q7_expected_distinct (
    code char(4),
	name LongName
);
...
COPY q7_expected_distinct (code, name) FROM stdin;
1001	Cotutelle
